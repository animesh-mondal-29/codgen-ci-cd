...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="AndAI - Insiders"
  PRODUCT_CODE="AndAIInsiders"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="AndAI-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="AndAI-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
